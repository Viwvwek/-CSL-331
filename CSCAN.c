...
void main() {
    int n, i, head, disk_size, seek_time = 0;
    ...
    scanf("%d", &disk_size); // Total disk size
    ...
    // After reaching the end, go back to the beginning (circular behavior)
    if (head != disk_size - 1) {
        seek_time += abs(disk_size - 1 - requests[n - 1]); // Move to the last cylinder
        printf("%d -> ", disk_size - 1); // Move to the end of the disk
        seek_time += abs(0 - (disk_size - 1)); // Move to the beginning (circular)
        printf("%d -> ", 0); // Return to the start of the disk
    }

    // Process requests to the left of the head (after wrapping around)
